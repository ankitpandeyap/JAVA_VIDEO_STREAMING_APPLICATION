...
const MyVideosPage = () => {
  ...
  const fetchMyVideos = async () => {
    ...
    try {
      // Referring to your VideoController: @GetMapping("/my-videos")
  ...
  const handleUpdateVideoSubmit = async (e) => {
    ...
    try {
      // Referring to your VideoController: @PatchMapping("/{videoId}")
      // The backend's VideoUpdateRequest DTO expects 'videoName' and 'description'.

      await axiosInstance.patch(`/videos/${selectedVideo.videoId}`, {
        // <--- TO THIS LINE: Use PATCH and videoId
        videoName: editFormData.title, // <--- AND THIS LINE: Map frontend 'title' to backend 'videoName'
  ...
  const handleDeleteVideoConfirm = async () => {
    ...
    try {
      // Referring to your VideoController: @DeleteMapping("/{videoId}")
      await axiosInstance.delete(`/videos/${selectedVideo.videoId}`); // <--- TO THIS LINE: Use videoId
        ...
        <div className="my-videos-content-area">
          {loading ? (
            ...
            <div className="video-grid">
              {myVideos.map((video) => (
                <VideoCard
                  key={video.videoId} // <--- TO THIS LINE: Use videoId for key
                  video={video}
      ...
      {showDeleteModal && selectedVideo && (
        <Modal title="Confirm Deletion" onClose={closeDeleteModal}>
          <p className="modal-body-text">
            ...
            <strong>{selectedVideo.videoName}</strong>"?{" "}
            {/* <--- TO THIS LINE: Use videoName */}
          ...
          <div className="modal-actions">
            ...
            <button
              ...
              disabled={deletingVideo}
            >
              {deletingVideo ? <LoadingSpinner small={true} /> : "Delete Video"}{" "}
              {/* Small spinner on button */}
            </button>
          </div>
        </Modal>
