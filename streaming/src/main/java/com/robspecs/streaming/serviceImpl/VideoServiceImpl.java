...
public class VideoServiceImpl implements VideoService {
    ...
    public Video uploadVideo(VideoUploadDTO v, User user) {
        ...
        if (!Objects.requireNonNull(file.getContentType()).startsWith("video/")) {
            logger.warn("Attempt to upload non-video file type: {} by user: {}", file.getContentType(), user.getUsername());
        ...
        // Send message to Kafka. The key can be videoId (as String) for partitioning/ordering
    ...
    public VideoDetailsDTO getVideo(Long videoId, User user) {
        ...
        if (!video.getUploadUser().getUserId().equals(user.getUserId()) && user.getRole() != com.robspecs.streaming.enums.Roles.ADMIN) {
             logger.warn("User {} attempted to access video {} which they do not own and are not admin.", user.getUsername(), videoId);
    ...
    public Video getVideoByFilePath(String relativeFilePath) {
        ...
        // where you know the exact relative path to a video file (e.g., HLS master playlist).
        // You might need to query the database to find the Video entity associated with this path.
        // For example, if 'relativeFilePath' is 'users/username/videos/processed/videoId/hls/master.m3u8',
        // you might extract 'videoId' from it and then fetch the Video.
        // For simplicity, let's assume 'relativeFilePath' directly corresponds to `originalFilePath`
        ...
        // 2. Adding a new method to VideoRepository: `findByOriginalFilePath(String path)`
        // 3. Or, if this is for HLS, you'd be looking up the video by its ID, and then finding
        ...
        // until you decide how to map a generic relativeFilePath back to a Video entity.
        // In the VideoController's stream method, you'd ideally get the Video entity by ID first,
        // then use `video.getResolutionFilePaths().get("hls_master")` to get the path.

        logger.warn("getVideoByFilePath called with path: {}. This method needs proper implementation.", relativeFilePath);
        throw new UnsupportedOperationException("getVideoByFilePath is not fully implemented yet. Please fetch video by ID and resolve paths from entity properties.");
        // Example: If you passed videoId to it, you would do:
        // return videoRepository.findById(videoId).orElseThrow(() -> new FileNotFoundException("Video not found."));
    }
