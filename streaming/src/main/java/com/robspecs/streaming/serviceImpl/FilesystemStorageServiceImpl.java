...
public class FilesystemStorageServiceImpl implements FileStorageService {
    ...
    public FilesystemStorageServiceImpl(@Value("${files.video.base-path}") String videoBasePath) {
        // Normalize the base path to ensure consistency (e.g., handles trailing slashes)
    ...
    @Override
    public String storeFile(InputStream inputStream, String fileName, String username, String typeSubdirectory) throws IOException {
        ...
        // Construct the target directory path: base_path/username/videos/[typeSubdirectory]/
        Path targetDirectory = this.fileStorageLocation
                .resolve(cleanUsername)
                .resolve("videos"); // Fixed 'videos' subdirectory
        ...
        try {
            Files.createDirectories(targetDirectory); // Create user-specific and type-specific directories if they don't exist
        } catch (IOException ex) {
            throw new FileStorageException("Could not create the directory for storing the file: " + targetDirectory, ex);
        }
        ...
        try {
            ...
            return Paths.get(this.videoBasePath) // Start from the configured base path string
    ...
    public Path getFilePath(String relativeFilePath) {
        ...
        if (!fullPath.startsWith(this.fileStorageLocation)) {
            throw new FileStorageException("Attempted to access file outside of storage directory: " + relativeFilePath);
        }
        return fullPath;
    }
