...
public class FFmpegService {
    ...
    /**
     * Retrieves media information (like duration, resolution) of a video file using Bytedeco.
     * @param videoPath The absolute Path to the video file.
     * @return VideoMetadata object containing video metadata.
     * @throws VideoProcessingException if an error occurs during media info retrieval.
     */
    public VideoMetadata getMediaInfo(Path videoPath) {
        ...
        // Set FFmpeg executable path if provided (optional, Bytedeco usually auto-detects)
        if (ffmpegExecutablePath != null && !ffmpegExecutablePath.isEmpty()) {
            logger.debug("ffmpegExecutablePath is set to {}. Bytedeco usually auto-detects, but this can be useful for debugging.", ffmpegExecutablePath);
        }
        ...
        try (FFmpegFrameGrabber grabber = new FFmpegFrameGrabber(videoPath.toString())) {
            ...
            // If both are -1 (meaning no video or audio stream or bitrate not available), totalBitRate remains 0.
            ...
            logger.error("An unexpected error occurred while getting media info for {}: {}", videoPath, e.getMessage(), e);
            throw new VideoProcessingException("An unexpected error occurred while getting media info for " + videoPath, e);
        }
    }
     ...
     * @return The relative path to the master HLS playlist (e.g., "users/username/videos/processed/videoId/hls/master.m3u8").
    ...
    public String transcodeToHLS(Path originalVideoPath, Long videoId, String username) {
        ...
        Path outputDirPath = fileStorageService.createDirectory(
            "users", username, "videos", "processed", String.valueOf(videoId), "hls"
        );
        ...
        try {
            ...
            // *** HLS specific options via setOption() - This is where you gain granular control ***
            ...
            // recorder.setOption("hls_segment_filename", outputDirPath.resolve("segment_%03d.ts").toString());
            // Example: To ensure segments are deleted when the playlist is updated (for live streams)
            ...
            while ((frame = grabber.grab()) != null) {
                ...
                // if (frameCount % 100 == 0) {
                //     logger.debug("Transcoding progress: {} frames processed for video {}", frameCount, videoId);
                // }
            }

            long endTime = System.currentTimeMillis();
            logger.info("Transcoding loop completed for video {}. Total frames: {}. Time taken: {} ms",
                        videoId, frameCount, (endTime - startTime));

            logger.info("HLS transcoding completed for video {}. Master playlist: {}", videoId, targetMasterPlaylist.getAbsolutePath());
            ...
            logger.error("An unexpected error occurred during HLS transcoding for video {}: {}", videoId, e.getMessage(), e);
