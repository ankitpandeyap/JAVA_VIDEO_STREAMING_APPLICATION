...
public class VideoProcessorService {
	...
	public void processVideoRequest(VideoProcessingRequest request) {
		...
		try {
			...
			// 1. Get video metadata (duration) using FFmpegService (now returns
			// VideoMetadata)
			VideoMetadata mediaInfo = (VideoMetadata) ffmpegService.getMediaInfo(originalVideoAbsolutePath); // Cast to
																												// VideoMetadata
			...
			if (request.getFileSize() <= SMALL_VIDEO_SIZE_BYTES) {
				...
				logger.info("Video {} ({}MB) is small, no transcoding. Marking as READY.", video.getVideoId(),
				...
				logger.info("Video {} ({}MB) is medium, no transcoding. Marking as READY.", video.getVideoId(),
						request.getFileSize() / (1024.0 * 1024.0));
				resolutionFilePaths.put("original", request.getOriginalFilePath()); // Use "original" key
				video.setStatus(VideoStatus.READY);
			} else {
				...
				String hlsMasterPlaylistPath = ffmpegService.transcodeToHLS(originalVideoAbsolutePath,
						video.getVideoId(), request.getUploadUserEmailOrUsername() // Assuming username is sufficient
